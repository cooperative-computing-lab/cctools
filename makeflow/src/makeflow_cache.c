...
void makeflow_cache_populate(struct dag *d, struct dag_node *n, struct list *outputs, struct batch_job_info *info) {
  ...
  caching_file_path = string_combine_multi(caching_file_path, "/jobs/", caching_prefix, "/", n->cache_id + 2, "/outputs", 0);
  ...
  caching_file_path = xxstrdup(d->caching_directory);
  caching_file_path = string_combine_multi(caching_file_path, "/jobs/", caching_prefix, "/", n->cache_id + 2, "/input_files", 0);
  sucess = create_dir(caching_file_path, 0777);
  if (!sucess) {
  ...
  descendant_directory_path = string_combine_multi(descendant_directory_path, "/jobs/", caching_prefix, "/", n->cache_id + 2, "/descendants", 0);
  ...
  ancestor_directory_path = string_combine_multi(ancestor_directory_path, "/jobs/", caching_prefix, "/", n->cache_id + 2, "/ancestors", 0);
  ...
  caching_file_path = xxstrdup(d->caching_directory);
  caching_file_path = string_combine_multi(caching_file_path, "/jobs/", caching_prefix, "/", n->cache_id + 2, 0);
  ...
  while((f = list_next_item(outputs))) {
    ...
    output_file_path = string_combine_multi(output_file_path, "/jobs/", caching_prefix, "/", n->cache_id + 2, 0);
  ...
  if (set_size(n->ancestors) == 0) {
    ...
    source_makeflow_file_path = string_combine_multi(source_makeflow_file_path, "/jobs/", caching_prefix, "/", n->cache_id + 2, "/source_makeflow", 0);
  ...
  while ((f=list_next_item(n->source_files))) {
    ...
    if (f->created_by == 0 && f->cache_path == NULL) {
      ...
      input_file= string_combine_multi(input_file, "/jobs/", caching_prefix, "/", n->cache_id + 2, "/input_files/", f->filename, 0);
      ...
      if (f->cache_path != NULL) {
        ...
        ancestor_output_file_path= string_combine_multi(ancestor_output_file_path, "/jobs/", caching_prefix, "/", ancestor->cache_id + 2, 0);
...
int makeflow_cache_copy_preserved_files(struct dag *d, struct dag_node *n, struct list *outputs) {
  ...
  while((f = list_next_item(outputs))) {
    output_file_path = xxstrdup(d->caching_directory);
    filename = xxstrdup("./");
    output_file_path = string_combine_multi(output_file_path, "/jobs/", caching_prefix, "/", n->cache_id + 2, "/outputs/" , f->filename, 0);
...
int makeflow_cache_is_preserved(struct dag *d, struct dag_node *n, char *command, struct list *inputs, struct list *outputs) {
  ...
  while ((f=list_next_item(outputs))) {
    ...
    filename = string_combine_multi(filename, "/jobs/", caching_prefix, "/", n->cache_id + 2, "/outputs/", f-> filename, 0);
...
void write_descendant_link(struct dag *d, struct dag_node *current_node, struct dag_node *ancestor_node) {
  ...
  descendant_job_path = string_combine_multi(descendant_job_path, "/jobs/", current_node_caching_prefix, "/", current_node->cache_id + 2, 0);
...
void write_ancestor_links(struct dag *d, struct dag_node *current_node, struct dag_node *ancestor_node) {
  ...
  ancestor_job_path = string_combine_multi(ancestor_job_path, "/jobs/", ancestor_node_caching_prefix, "/", ancestor_node->cache_id + 2, 0);
