...
/* Allocates and initializes a new dag structure. Returns a
 * pointer to a new struct dag, or NULL on failure */
...
char *dag_parse_readline(struct dag_parse *bk, struct dag_node *n)
{
	...
	char *raw_line = get_line(bk->dagfile);
...
int dag_parse_variable(struct dag_parse *bk, struct dag_node *n, char *line)
{
	...
	if(append && old_value) {
		...
		hash_table_insert((n ? n->variables : bk->d->variables), name, xxstrdup(value));
	}
