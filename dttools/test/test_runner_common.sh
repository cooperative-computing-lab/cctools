...
run_wq_worker()
{
	...
	echo "Running worker."
	echo "$WORK_QUEUE_WORKER --single-shot --timeout=10s --cores ${cores:-1} --memory ${memory:-250} --disk ${disk:-250} --gpus ${gpus:-0} ${coprocess:-} ${coprocess_cores:-} ${coprocess_disk:-} ${coprocess_memory:-} ${coprocess_gpus:-} --debug=all --debug-file=$log $* localhost $(cat $port_file)"
	if ! "$WORK_QUEUE_WORKER" --single-shot --timeout=10s --cores ${cores:-1} --memory ${memory:-250} --disk ${disk:-250} --gpus ${gpus:-0} ${coprocess:-} ${coprocess_cores:-} ${coprocess_disk:-} ${coprocess_memory:-} ${coprocess_gpus:-} --debug=all --debug-file="$log" $* localhost $(cat "$port_file"); then
