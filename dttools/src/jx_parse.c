...
retry:
	...
	if (isspace(c)) {
		...
		char d = jx_getchar(s);
		if (d == '&')
			return JX_TOKEN_C_AND;
		jx_parse_error_c(s, "single & must be && instead");
		return JX_TOKEN_PARSE_ERROR;
	} else if (c == '|') {
		char d = jx_getchar(s);
		if (d == '|')
			return JX_TOKEN_C_OR;
		jx_parse_error_c(s, "single | must be || instead");
		return JX_TOKEN_PARSE_ERROR;
	} else if (c == '\"') {
		int i;
...
static struct jx_comprehension *jx_parse_comprehension(struct jx_parser *s)
{
	...
	result = jx_comprehension(variable, elements, condition, jx_parse_comprehension(s));
	result->line = line;
	free(variable);
	return result;
...
static struct jx *jx_parse_atomic(struct jx_parser *s, bool arglist)
{
	...
	case JX_TOKEN_TRUE:
		return jx_add_lineno(s, jx_boolean(true));
	case JX_TOKEN_FALSE:
		return jx_add_lineno(s, jx_boolean(false));
	case JX_TOKEN_NULL:
