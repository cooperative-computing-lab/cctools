...
static struct jx *jx_eval_null(struct jx_operator *op, struct jx *left, struct jx *right)
{
	...
	case JX_OP_NE:
		return jx_boolean(0);
	default:
		FAILOP(op, jx_null(), jx_null(), "unsupported operator on null");
	}
}
...
static struct jx *jx_eval_boolean(struct jx_operator *op, struct jx *left, struct jx *right)
{
	...
	case JX_OP_NOT:
		return jx_boolean(!b);
	default:
		FAILOP(op, jx_copy(left), jx_copy(right), "unsupported operator on boolean");
	}
}
...
static struct jx *jx_eval_integer(struct jx_operator *op, struct jx *left, struct jx *right)
{
	...
	case JX_OP_DIV:
		if (b == 0)
			FAILOP(op, jx_copy(left), jx_copy(right), "division by zero");
		return jx_integer(a / b);
	case JX_OP_MOD:
		if (b == 0)
			FAILOP(op, jx_copy(left), jx_copy(right), "division by zero");
		return jx_integer(a % b);
	default:
		FAILOP(op, jx_copy(left), jx_copy(right), "unsupported operator on integer");
	}
}
...
static struct jx *jx_eval_double(struct jx_operator *op, struct jx *left, struct jx *right)
{
	...
	default:
		FAILOP(op, jx_copy(left), jx_copy(right), "unsupported operator on double");
	}
}
...
static struct jx *jx_eval_string(struct jx_operator *op, struct jx *left, struct jx *right)
{
	...
	default:
		FAILOP(op, jx_copy(left), jx_copy(right), "unsupported operator on string");
	}
}
...
static struct jx *jx_eval_array(struct jx_operator *op, struct jx *left, struct jx *right)
{
	assert(op);
	if (!(left && right))
		FAILOP(op, jx_copy(left), jx_copy(right), "missing arguments to array operator");
	...
	default:
		FAILOP(op, jx_copy(left), jx_copy(right), "unsupported operator on array");
	}
}
...
static struct jx *jx_eval_slice(struct jx *array, struct jx *slice)
{
	...
	if (array->type != JX_ARRAY) {
		return jx_error(jx_format("on line %d, only arrays support slicing", right->line));
	}
	if (left && left->type != JX_INTEGER)
		FAILOP((&slice->u.oper), jx_copy(left), jx_copy(right), "slice indices must be integers");
	if (right && right->type != JX_INTEGER)
		FAILOP((&slice->u.oper), jx_copy(left), jx_copy(right), "slice indices must be integers");
	...
	for (jx_int_t i = start; i < end; ++i) {
		...
		if (j)
			jx_array_append(result, jx_copy(j));
	}
...
static struct jx *jx_eval_lookup(struct jx *left, struct jx *right)
{
	...
	if (left->type == JX_OBJECT && right->type == JX_STRING) {
		...
		if (r) {
			...
			return jx_error(jx_format("lookup on line %d, key not found", right->line));
		}
		...
		struct jx *err = jx_error(jx_format("on line %d, %s: invalid type for lookup", right->line, s));
		free(s);
		return jx_error(err);
	}
}
...
static struct jx *jx_eval_operator(struct jx_operator *o, struct jx *context)
{
	...
	if (o->type == JX_OP_CALL)
		return jx_eval_call(o->left, o->right, context);
	...
	if (o->type == JX_OP_DOT)
		return jx_eval_dot(o, left, o->right, context);
	...
	if (o->type == JX_OP_SLICE)
		return jx_operator(JX_OP_SLICE, left, right);
	...
	case JX_ARRAY:
		...
		break;
	default:
		FAILOP(o, left, right, "rvalue does not support operators");
	}
...
static struct jx_item *jx_eval_list_comprehension(struct jx *body, struct jx_comprehension *comp, struct jx *context)
{
	...
	if (jx_istype(list, JX_ERROR))
		return jx_item(list, NULL);
	if (!jx_istype(list, JX_ARRAY)) {
		jx_delete(list);
		return jx_item(jx_error(jx_format("on line %d: list comprehension takes an array", comp->line)), NULL);
	}
...
static struct jx_item *jx_eval_item(struct jx_item *item, struct jx *context)
{
	...
	if (item->comp) {
		...
		return jx_item(jx_eval(item->value, context), jx_eval_item(item->next, context));
	}
}
...
static struct jx *jx_check_errors(struct jx *j)
{
	...
	case JX_OBJECT:
		for (struct jx_pair *p = j->u.pairs; p; p = p->next) {
			if (jx_istype(p->key, JX_ERROR))
				err = jx_copy(p->key);
			if (!err && jx_istype(p->value, JX_ERROR))
				err = jx_copy(p->value);
			if (err) {
				jx_delete(j);
				return err;
...
struct jx *jx_eval(struct jx *j, struct jx *context)
{
	...
	case JX_SYMBOL: {
		...
		if (t) {
			...
			return jx_error(jx_format("on line %d, %s: undefined symbol", j->line, j->u.symbol_name));
		}
	}
	case JX_DOUBLE:
	case JX_BOOLEAN:
...
void jx_eval_enable_external(int enable) { __jx_eval_external_functions_flag = enable; }
