	...
	while ((item = jx_iterate_array(j, &i))) {
		...
		if (t == (jx_type_t)JX_ANY) {
			*((struct jx **)out) = jx_copy(item);
			...
			switch (t) {
			case JX_INTEGER:
				if (!jx_match_integer(item, out))
					goto DONE;
				break;
			case JX_BOOLEAN:
				if (!jx_match_boolean(item, out))
					goto DONE;
				break;
			case JX_DOUBLE:
				if (!jx_match_double(item, out))
					goto DONE;
				break;
			case JX_STRING:
				if (!jx_match_string(item, out))
					goto DONE;
				break;
			case JX_SYMBOL:
				if (!jx_match_symbol(item, out))
					goto DONE;
				break;
			case JX_OBJECT:
				if (!jx_istype(item, JX_OBJECT))
					goto DONE;
				if (!(*((struct jx **)out) = jx_copy(item)))
					goto DONE;
				break;
			case JX_ARRAY:
				...
				break;
			case JX_OPERATOR:
				if (!jx_istype(item, JX_OPERATOR))
					goto DONE;
				if (!(*((struct jx **)out) = jx_copy(item)))
					goto DONE;
				break;
			case JX_NULL:
				if (!jx_istype(item, JX_NULL))
					goto DONE;
				if (!(*((struct jx **)out) = jx_copy(item)))
					goto DONE;
				break;
