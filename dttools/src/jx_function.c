...
static char *jx_function_format_value(char spec, struct jx *args)
{
	...
	case 'i':
		if (jx_istype(j, JX_INTEGER))
			result = string_format("%" PRIi64, j->u.integer_value);
		break;
	case 'e':
...
struct jx *jx_function_join(struct jx *args)
{
	...
	for (size_t location = 0; (value = jx_array_shift(list)); location++) {
		...
		if (location > 0) {
			if (delimeter)
				result = string_combine(result, delimeter->u.string_value);
			else
...
struct jx *jx_function_listdir(struct jx *args)
{
	...
	if (!d) {
		out = make_error(func, args, "%s, %s", a->u.string_value, strerror(errno));
		goto FAILURE;
	}
...
static struct jx *expand_template(struct jx *template, struct jx *ctx, struct jx *overrides)
{
	...
	while (*s) {
		...
		if (*s == 0) {
			out = make_error(func, template, "unterminated template expression");
			goto FAILURE;
		}
		if (!isalpha(*s) && *s != '_') {
			out = make_error(func,
		...
		case JX_STRING:
			...
			break;
		default:
			out = make_error(func, template, "cannot format expression in template");
			goto FAILURE;
		}
		buffer_rewind(&var, 0);
	}
...
struct jx *jx_function_fetch(struct jx *args)
{
	...
	if (string_match_regex(path, "http(s)?://")) {
		...
		free(cmd);
		if (!stream) {
			result = make_error(func, args, "error fetching %s: %s", path, strerror(errno));
			goto FAILURE;
		}
...
struct jx *jx_function_select(struct jx *args, struct jx *ctx)
{
	...
	for (void *i = NULL; (item = jx_iterate_array(objlist, &i));) {
		...
		if (!jx_istype(j, JX_BOOLEAN)) {
			jx_delete(result);
			result = make_error(func, args, "select expression must use a boolean predicate");
			goto FAILURE;
		}
		if (j->u.boolean_value)
			jx_array_append(result, jx_copy(item));
		jx_delete(j);
		jx_delete(new_ctx);
		j = NULL;
		new_ctx = NULL;
	...
	for (void *i = NULL; (item = jx_iterate_array(objlist, &i));) {
		...
		for (void *j = NULL; (key = jx_iterate_keys(item, &j));) {
			if (!jx_lookup(result, key)) {
				...
				jx_insert(result, jx_string(key), jx_string(jx_type_string(lookup->type)));
			}
		}
	}
