...
void bitmap_rotate_clockwise( struct bitmap *s, struct bitmap *t )
{
	...
	if(s->width!=t->height || s->height!=t->width) return;
...
void bitmap_subset( struct bitmap *m, int origx, int origy, struct bitmap *n )
{
	...
	while(origx<0)		origx=origx+m->width;
	while(origy<0)		origy=origy+m->height;
	while(origx>=m->width)	origx-=m->width;
	while(origy>=m->height)	origy-=m->height;
...
struct bitmap * bitmap_load_sgi_rgb( const char *path )
{
	...
	if(doswap) swap_short(&header,sizeof(header));
	...
	if(start_table) free(start_table);
	if(length_table) free(length_table);
