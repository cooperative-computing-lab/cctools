...
ddb_collect: Returns log data for the specified time period. The script takes three arguments. The first argument is a directory where the catalog history files are stored in sub-directories by year. The second argument specifies the start time as a timestamp, date or time in the format YYYY-MM-DD-HH-MM-SS. The third argument (optional) can be a timestamp, duration, or date and time of the same format YYYY-MM-DD-HH-MM-SS. The duration can be in seconds (+180 or s180), minutes (m45), hours (h8), days (d16), weeks (w3), or years (y2). 
ddb_select_*: Filters streamed data (through standard input) and returns only the objects which satisfy the provided arguments. For example, type=wq_master would only output objects with type wq_master. There are three select tools: ddb_select_static, ddb_select_dynamic, ddb_select_complete. The man pages describe the differences between these tools in more detail. 
ddb_project: Defines desired attributes from a stream of delta logs (standard input), removing the remaining attributes, but no objects. 
ddb_reduce_temporal Aggregates attribute values (from standard input) over a specified time span. 
ddb_reduce_spatial Summarizes attribute values (from standard input) over all objects, returning a single object for each timestamp. 
ddb_pivot Transforms the data into a more plottable format. 
...
ddb_collect /data/catalog.history 2013-02-1@00:00:00 d365 | \
