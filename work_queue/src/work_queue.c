...
struct work_queue_worker {
	...
	int worker_fetch_file_port;
	struct work_queue_workers_fetch *ongoing_fetch;
...
work_queue_msg_code_t process_provide_port(struct work_queue_worker *w, char *line) {
	char portnum[WORK_QUEUE_LINE_MAX];
	int n = sscanf(line, "worker_port = %s\n", portnum);
	if (n != 1) {
		return MSG_FAILURE;
	}

	w->worker_fetch_file_port = atoi(portnum);
...
static work_queue_result_code_t send_input_files( struct work_queue *q, struct work_queue_worker *w, struct work_queue_task *t )
{
	...
	if(t->input_files) {
		...
		while((f = list_next_item(t->input_files))) {
			...
			if(result != SUCCESS && result != FETCH_SENT) return result;
		}
	}
...
static struct work_queue_worker *find_worker_to_fetch_file_from(struct work_queue *q, const char *local_name, const char *remote_name, struct work_queue_worker *wk, struct work_queue_task *t) {
	...
	while(hash_table_nextkey(q->worker_table, &key, (void**)&w)) {
		...
		if (is_valid_fetch(q, potential_fetch)) {
			...
			free(potential_fetch->remote_name);
			free(potential_fetch->local_name);
