	...
	if (recv_master_message(master, line, sizeof (line), idle_stoptime)) {
		if (sscanf(line, "task %" SCNd64, &taskid) == 1) {
			...
			if (pattern_match(line, "^put (.+) (%d+) ([0-7]+) (%d+)$", &f, &l, &m, &g) >= 0) {
				strncpy(filename, f, WORK_QUEUE_LINE_MAX);
				free(f);
				length = strtoll(l, 0, 10);
				free(l);
				mode = strtol(m, NULL, 8);
				free(m);
...
static void show_help(const char *cmd) {
	...
	printf("%-30s Initialize as MPI programs (requires being built with --mpi in cctools configuration).\n", "--mpi");
}

enum {
	LONG_OPT_DEBUG_FILESIZE = 256, LONG_OPT_VOLATILITY, LONG_OPT_BANDWIDTH,
