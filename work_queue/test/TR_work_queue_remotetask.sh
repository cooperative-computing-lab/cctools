...
run_wq_worker_coprocess()
{
	...
	echo "Running worker."
	if ! "$WORK_QUEUE_WORKER" --single-shot --timeout=10s --coprocess ${coprocess} --coprocess_cores 1 --coprocess_disk 2000 --coprocess_memory 2000 --debug=all --debug-file="$log" $* localhost $(cat "$port_file"); then
