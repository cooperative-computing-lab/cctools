...
run_wq_worker_coprocess()
{
	...
	echo "Running worker."
	if ! "$WORK_QUEUE_WORKER" --single-shot --timeout=10s --coprocess ${coprocess} --coprocess_cores 2 --coprocess_disk 4000 --coprocess_memory 4000 --coprocess_gpus 0 --debug=all --debug-file="$log" $* localhost $(cat "$port_file"); then
