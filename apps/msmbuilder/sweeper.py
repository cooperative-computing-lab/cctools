...
class Sweeper:
    ...
    def sqldbsubmit(self, host, user):
        ...
        os.system(('mysql --debug-check --show-warnings -h %s -u %s --password=%s < sqlscript') % (host, user, open('secret/mysql.pwd').read().strip()))
        sys.exit(0)
