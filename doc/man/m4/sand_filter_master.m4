...
SECTION(SYNOPSIS)
CODE(sand_filter_master [options] sequences.cfa candidates.cand)
...
OPTIONS_BEGIN
OPTION_ARG_SHORT(p,port)Port number for queue master to listen on. (default: 9123)
OPTION_ARG_SHORT(s,size)Number of sequences in each filtering task. (default: 1000)
OPTION_ARG_SHORT(r,file)A meryl file of repeat mers to be filtered out.
OPTION_ARG_SHORT(R,n)Automatically retry failed jobs up to n times. (default: 100)
OPTION_ARG_SHORT(k,size)The k-mer size to use in candidate selection (default is 22).
OPTION_ARG_SHORT(w,size)The minimizer window size. (default is 22).
OPTION_FLAG_SHORT(u)If set, do not unlink temporary binary output files.
OPTION_ARG_SHORT(c,file)Checkpoint filename; will be created if necessary.
OPTION_ARG_SHORT(d,flag)Enable debugging for this subsystem.  (Try BOLD(-d all) to start.)
OPTION_ARG_SHORT(F,number)Work Queue fast abort multiplier.     (default is 10.)
...
OPTION_FLAG_SHORT(h)Show this help screen
OPTIONS_END
