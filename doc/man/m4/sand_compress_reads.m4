...
SECTION(SYNOPSIS)
CODE(sand_compress_reads [-qcivh] [infile] [outfile])
...
OPTIONS_BEGIN
OPTION_FLAG_SHORT(q)Quiet mode: suppress summary line.
OPTION_FLAG_SHORT(c)Remove Celera read ids if data came from the Celera gatekeeper.
OPTION_FLAG_SHORT(i)Remove Celera read ids but leave internal ids if the data came from the Celera gatekeeper.
OPTION_FLAG_SHORT(h)Display version information.
OPTION_FLAG_SHORT(v)Show help text.
OPTIONS_END
