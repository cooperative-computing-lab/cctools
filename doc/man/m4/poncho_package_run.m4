...
CODE(BOLD(poncho_package_run [options] --environment PARAM(file) PARAM(command and args ...)))
...
OPTION_ITEM(command and args)           Command to execute inside the given environment.
OPTIONS_END
SECTION(EXIT STATUS)
