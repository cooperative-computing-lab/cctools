...
SUBSECTION(Lambda Options)
OPTIONS_BEGIN
OPTION_PAIR(--lambda-config,path) Path to the configuration file generated by batch_job_lambda_setup.sh
OPTIONS_END

SUBSECTION(batch_job_lambda_setup.sh Options)
OPTIONS_BEGIN
OPTION_PAIR(-r,role-name) AWS Identity and Access Management Role (IAM Role) for Lambda function execution. Creates if necessary. (default is 'Makeflow-Role')
OPTION_PAIR(-b,bucket-name) AWS S3 bucket to hold objects during Lambda function execution. Creates if necessary. (default is '$USER-Makeflow-Bucket')
OPTION_PAIR(-f,function-name) AWS Lambda function to execute jobs. Creates if necessary. (default is 'Makeflow-Function')
OPTION_PAIR(-g,region-name) Use this AWS Region. (default is 'us-east-1')
OPTION_PAIR(-p,profile-name) Use this specific profile from your credential file. (default is 'default')
OPTION_PAIR(-o,path) Place the output here. (default is 'lambda_config.json').
OPTION_ITEM(-h) Show the help screen.
OPTIONS_END

SUBSECTION(batch_job_lambda_teardown.sh Options)
OPTIONS_BEGIN
OPTION_ITEM(-r) Preserve the Identity and Access Management Role (IAM Role).
OPTION_ITEM(-b) Preserve the AWS S3 bucket.
OPTION_ITEM(-f) Preserve the AWS Lambda function.
