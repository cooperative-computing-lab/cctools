...
workdir_footprint:         [size in bytes of all working directories in the tree, int]
LONGCODE_END
...
workdir_footprint         [current byte size of working directories in the tree,   int]
LONGCODE_END
...
To monitor 'sleep 10', at 2 second intervals, with output to sleep-log, sleep-log-time-series, and sleep-log-opened, and with a monitor alarm at 5 seconds:

LONGCODE_BEGIN
% resource_monitor --interval=2 -L"wall_time: 5" -o sleep-log -- sleep 10
LONGCODE_END

It can be run automatically from makeflow, by specifying the '-M' flag:
...
Instead of creating the queue with CODE(work_queue_create), the
call:

LONGCODE_BEGIN
work_queue_create_monitoring(port, some-log-file)
LONGCODE_END
...
LIST_BEGIN
LIST_ITEM IO and resident set size are not monitored in FreeBSD and Darwin.
