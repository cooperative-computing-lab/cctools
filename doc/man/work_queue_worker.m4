...
OPTION_PAIR(-m, mode)Choose the worker mode.  Can be [w]orker, [f]oreman, [c]lassic or [a]uto (default=auto).
...
OPTION_PAIR(--cores, n)Manually set the number of cores this worker should use.
...
setting the mode directly with the BOLD(-m PARAM([f]oreman)) option.  The foreman can be directed to advertise its
...
LONGCODE_BEGIN
% work_queue_worker -m foreman -M project_A -N foreman_A1 -f 9123
LONGCODE_END
