...
SECTION(OPTIONS)
OPTIONS_BEGIN
OPTION_ITEM(`-a, --advertise')Enable auto mode. In this mode the worker would ask a catalog server for available masters.
...
OPTION_TRIPLET(-N, name, name)Set the project name of this foreman to PARAM(project).  If in worker, classic or auto mode, behaves as BOLD(-M) for backwards compatibility.
