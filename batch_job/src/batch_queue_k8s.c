		...
		const struct rmsummary *resources)
{
	...
	if (pid > 0) {
		...
		k8s_job_info *curr_job_info = create_k8s_job_info(job_id, cmd, extra_input_files, extra_output_files);
		...
		fprintf(fd, k8s_config_tmpl, mf_uuid->str, pod_id, pod_id, k8s_image, resources_block, job_id, pod_id);
		...
		execlp("/bin/sh", "sh", k8s_script_file_name, "create", pod_id, job_id_str, extra_input_files, cmd, extra_output_files, (char *)NULL);
		_exit(127);
...
static int batch_queue_k8s_remove(struct batch_queue *q, batch_queue_id_t jobid)
{
	...
	if (pid > 0) {
		...
		debug(D_BATCH, "Trying to remove task %d by deleting pods %s.", (int)jobid, pod_id);
		...
		struct batch_job_info *info_out, time_t stoptime)
{
	...
	while (1) {
		...
		if (failed_job_id > 0) {
			...
			batch_queue_k8s_handle_complete_task(pod_id, failed_job_id, curr_k8s_job_info, 0, 1, info_out, running_pod_lst, q);
			free(pod_id);
			return failed_job_id;
		}
			...
			if (strcmp(task_state, "pod_created") == 0) {
				...
				if (curr_k8s_job_info->is_running == 0) {
					...
					if (pid > 0) {
						...
						debug(D_BATCH, "run job %s: %s in pod %s with pid %ld", curr_job_id, curr_k8s_job_info->cmd, curr_pod_id, (long)pid);

					} else if (pid == 0) {

						execlp("/bin/sh", "sh", k8s_script_file_name, "exec", curr_pod_id, curr_job_id, curr_k8s_job_info->extra_input_files, curr_k8s_job_info->cmd, curr_k8s_job_info->extra_output_files, (char *)NULL);
						_exit(127);
				...
				batch_queue_k8s_handle_complete_task(curr_pod_id, curr_job_id_int, curr_k8s_job_info, 1, 0, info_out, running_pod_lst, q);
				return curr_job_id_int;

			} else if (strcmp(task_state, "exec_failed") == 0) {
				int exit_code_int = atoi(strtok(NULL, ","));
				batch_queue_k8s_handle_complete_task(curr_pod_id, curr_job_id_int, curr_k8s_job_info, 0, exit_code_int, info_out, running_pod_lst, q);
				return curr_job_id_int;
