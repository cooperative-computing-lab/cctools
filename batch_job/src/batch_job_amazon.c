...
int compare_instance_type(struct idle_instance* instance, const char* type_2){
...
int push_back_idle_instance(struct idle_instance* instance){
	...
	debug(D_BATCH,"adding instance %s to the list, current list count is %d\n",
...
static const char * fetch_idle_instance(const char* instance_type){
	...
	debug(D_BATCH,"entering fetch_idle_instance, current list count is %d\n", list_size(instance_list));
	if (list_size(instance_list) == 0){
		debug(D_BATCH,"idle instance list empty\n");
		return 0;
	}
	struct idle_instance* i = list_find(instance_list, (list_op_t) compare_instance_type, instance_type);
	if (!i){
		debug(D_BATCH,"could not find idle instance of type %s in the list\n", instance_type);
		return 0;
	}
	const char* instance_id = strdup(i->instance_id);
	list_remove(instance_list, i);
	free(i);
	debug(D_BATCH,"erasing instance %s from list \n", instance_id);
	debug(D_BATCH,"exiting fetch_idle_instance, current list count is %d\n", list_size(instance_list));
	return instance_id;
}
...
static int idle_instance_expire( struct idle_instance *i, int *timediff )
{
...
/*
Recursively terminate expired idle instances based on difftime and timestamp
*/
...
static int terminate_expired_instances( struct aws_config *c, const int timediff )
{
	...
	debug(D_BATCH,"entering aws_terminate_idle_instance, current list count is %d\n", list_size(instance_list));
	if (list_size(instance_list) == 0){
		debug(D_BATCH,"idle instance list empty\n");
		return 0;
	}
	struct idle_instance* i = list_find(instance_list, (list_op_t) idle_instance_expire, timediff_p);
	while (i != NULL){
		aws_terminate_instance(c, i->instance_id);
		list_remove(instance_list, i);
		debug(D_BATCH,"erasing idle instance %s from list \n", i->instance_id);
		debug(D_BATCH,"exiting terminate_idle_instance, current list count is %d\n", list_size(instance_list));
		free(i);
		i = list_find(instance_list, (list_op_t) idle_instance_expire, timediff_p);
	}
	return 1;
}
