...
# Next thoughts: One large file that must be shared [so that the node hosting that file gets hosed by multiple clients eventually.]. Then, unique files.

TASKS = 25
SHARED = []
for i in (36,):
    SHARED.append({'count': 1, 'prefix': ('%dM-shared' % (2**(i-20))), 'size': functools.partial(lambda i: 2**i, i)})
for i in (32,31,30,29,28,):
    SHARED.append({'count': 25, 'prefix': ('%dM-shared' % (2**(i-20))), 'size': functools.partial(lambda i: 2**i, i)})
UNIQUE = []
