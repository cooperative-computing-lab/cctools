...
static int schedule_replication (confuga *C)
{
	...
	static const char SQL[] =
		...
		"		Replicas AS ("
		"				SELECT FileReplicas.id AS fid, FileReplicas.sid, FileReplicas.size, FileReplicas.minimum_replicas, FileReplicas.time_create"
		"					FROM Confuga.FileReplicas"
		"			UNION"
		"				SELECT File.id AS fid, TransferJob.tsid AS sid, File.size, File.minimum_replicas, File.time_create"
		...
		"				FROM Replicas"
		"				WHERE time_create < (strftime('%s', 'now'))-60"
		"				GROUP BY fid"
