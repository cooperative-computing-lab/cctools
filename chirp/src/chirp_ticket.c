...
void chirp_ticket_name(const char *ticket_dir, const char *pk, char *ticket_subject, char *ticket_filename)
{
	...
	sprintf(ticket_filename, "%s/"TICKET_FILENAME_FORMAT, ticket_dir, md5_string(digest));
}

void chirp_ticket_filename(char *ticket_filename, const char *ticket_dir, const char *ticket_subject)
{
	const char *digest;
	int result = chirp_ticket_isticketsubject(ticket_subject, &digest);
	assert(result);
	sprintf(ticket_filename, "%s/"TICKET_FILENAME_FORMAT, ticket_dir, digest);
}
