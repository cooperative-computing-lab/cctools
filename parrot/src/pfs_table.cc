	...
	if(rlres > 0) {
		...
		if(link_target[0] != '/') {
			...
			if(basename_start) {
				...
				snprintf(absolute_link_target, PFS_PATH_MAX, "%*.*s%s", dirname_len, dirname_len, pname->path, link_target);
...
pfs_file *pfs_table::open_object(const char *lname, int flags, mode_t mode, int force_cache)
{
	...
	if(!strcmp(pfs_current->name, "cp") || !strcmp(string_back(pfs_current->name, 3), "/cp")
		) {
		force_stream = 1;
	}
...
int pfs_table::select(int n, fd_set * r, fd_set * w, fd_set * e, struct timeval *timeout)
{
	...
	for(i = 0; i < n; i++) {
		...
		int wantflags = 0;
		if(r && FD_ISSET(i, r))
			wantflags |= PFS_POLL_READ;
		if(w && FD_ISSET(i, w))
			wantflags |= PFS_POLL_WRITE;
		if(e && FD_ISSET(i, e))
			wantflags |= PFS_POLL_EXCEPT;
		if(!wantflags)
			continue;
	...
	if(result > 0) {
		...
		if(e)
			FD_ZERO(e);
		for(i = 0; i < n; i++) {
			if(r && FD_ISSET(i, &out_r))
				FD_SET(i, r);
			if(w && FD_ISSET(i, &out_w))
				FD_SET(i, w);
			if(e && FD_ISSET(i, &out_e))
				FD_SET(i, e);
		}
		...
		if(timeout) {
			...
			if((curtime.tv_sec > stoptime.tv_sec) || ((curtime.tv_sec == stoptime.tv_sec) && (curtime.tv_usec >= stoptime.tv_usec))
				) {
				result = 0;
				pfs_current->seltime.tv_sec = 0;
		...
		if(result == 0) {
			...
			for(i = 0; i < n; i++) {
				...
				if(r && FD_ISSET(i, r))
					flags |= PFS_POLL_READ;
				if(w && FD_ISSET(i, w))
					flags |= PFS_POLL_WRITE;
				if(e && FD_ISSET(i, e))
					flags |= PFS_POLL_EXCEPT;
				if(flags)
					pointers[i]->file->poll_register(flags);
			}
		}
	}
...
pfs_ssize_t pfs_table::pread(int fd, void *data, pfs_size_t nbyte, pfs_off_t offset)
{
	...
	if((fd < 0) || (fd >= pointer_count) || (pointers[fd] == 0)) {
		...
		if(!f->is_seekable() && f->get_last_offset() != offset) {
			...
			if(result > 0)
				f->set_last_offset(offset + result);
		}
	}
...
pfs_ssize_t pfs_table::readv(int fd, const struct iovec * vector, int count)
{
	...
	for(i = 0; i < count; i++) {
		...
		if(chunk < 0)
			return chunk;
		result += chunk;
		if(chunk != (pfs_ssize_t) vector->iov_len)
			return result;
		vector++;
	}
...
pfs_ssize_t pfs_table::copyfile(const char *source, const char *target)
{
	...
	if(resolve_name(source, &psource) < 0)
		return -1;
	if(resolve_name(target, &ptarget) < 0)
		return -1;
		...
		if(logical_addr >= m->logical_addr && (logical_addr < (m->logical_addr + m->map_length))) {
			...
			*p = m->next;
			...
			if(logical_addr > m->logical_addr) {
				mmap_create_object(m->file, m->file_offset, logical_addr - m->logical_addr, m->prot, m->flags);
			...
			if((logical_addr + length) < (m->logical_addr + m->map_length)) {
				mmap_create_object(m->file, m->file_offset + m->map_length - (m->logical_addr - logical_addr), m->map_length - length - (logical_addr - m->logical_addr), m->prot, m->flags);
