...
{
	BEGIN debug(D_LIBCALL, "open %s %u %u", path, flags, mode);
...
{
	BEGIN debug(D_LIBCALL, "close %d", fd);
...
{
	BEGIN debug(D_LIBCALL, "getsockname %d 0x%x 0x%x", fd, addr, addrlen);
...
{
	BEGIN debug(D_LIBCALL, "recvfrom %d 0x%x %d %d 0x%x 0x%x", fd, data, length, flags, addr, addrlength);
...
{
	BEGIN debug(D_LIBCALL, "recvmsg %d 0x%x %d", fd, msg, flags);
...
{
	BEGIN debug(D_LIBCALL, "sendto %d 0x%x %d %d 0x%x %d", fd, data, length, flags, addr, addrlength);
...
{
	BEGIN debug(D_LIBCALL, "setsockopt %d %d %d 0x%x %d", fd, level, option, value, length);
	result =::setsockopt(pfs_current->table->get_real_fd(fd), level, option, value, length);
END}

int pfs_shutdown(int fd, int how)
{
	BEGIN debug(D_LIBCALL, "shutdown %d %d", fd, how);
...
{
	BEGIN debug(D_LIBCALL, "timeout %s", str);
	if(str)
		pfs_master_timeout = string_time_parse(str);
	else if(isatty(0))
...
{
	BEGIN debug(D_LIBCALL, "mkalloc %s %lld %d", path, size, mode);
...
{
	BEGIN debug(D_LIBCALL, "search %s %s %d %p %zu", paths, pattern, flags, buffer, buffer_length);
...
{
	BEGIN debug(D_LIBCALL, "mmap_create %d %llx %llx %x %x", fd, file_offset, length, prot, flags);
...
{
	BEGIN debug(D_LIBCALL, "mmap_update %llx %llx", logical_address, channel_address);
...
{
	BEGIN debug(D_LIBCALL, "mmap_delete %llx %llx", logical_address, length);
...
{
	BEGIN debug(D_LIBCALL, "getxattr %s %s", path, name);
...
{
	BEGIN debug(D_LIBCALL, "lgetxattr %s %s", path, name);
...
{
	BEGIN debug(D_LIBCALL, "setxattr %s %s <> %zu %d", path, name, size, flags);
...
{
	BEGIN debug(D_LIBCALL, "lsetxattr %s %s <> %zu %d", path, name, size, flags);
