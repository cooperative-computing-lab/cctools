	...
	if(entering) {
		...
		if(p->syscall_result == 0) {
			...
			if(p->interrupted) {
				...
				if(rfd >= 0)
					pfs_poll_wakeon(rfd, PFS_POLL_READ);
			}
		} else {
		...
		if(actual_result > 0) {
			...
			if(p->syscall_result >= 0) {
				...
				if(errno == EAGAIN && !pfs_is_nonblocking(fd)) {
					...
					int rfd = pfs_get_real_fd(fd);
					if(rfd >= 0)
						pfs_poll_wakeon(rfd, PFS_POLL_WRITE);
				} else {
	...
	if(entering) {
		...
		if(p->syscall_result >= 0) {
			if(sixty_four) {
				...
				if(lbuf.f_blocks > 0xffffffff)
					lbuf.f_blocks = 0xffffffff;
				if(lbuf.f_bavail > 0xffffffff)
					lbuf.f_bavail = 0xffffffff;
				if(lbuf.f_bfree > 0xffffffff)
					lbuf.f_bfree = 0xffffffff;
		...
		case SYS_CONNECT:
			...
			if(r >= 0) {
				...
				if(p->interrupted) {
					...
					if(rfd >= 0)
						pfs_poll_wakeon(rfd, PFS_POLL_READ | PFS_POLL_WRITE | PFS_POLL_EXCEPT);
				}
			} else {
				divert_to_dummy(p, -errno);
			}
...
static void redirect_ldso(struct pfs_process *p, const char *ldso, INT64_T * args, char *const start_of_available_scratch)
{
	...
	if(strcmp(real_physical_name, ldso_physical_name) == 0)
		return;
...
void decode_execve(struct pfs_process *p, int entering, int syscall, INT64_T * args)
{
	...
	if(args[0] == 0) {
		...
		if(!strncmp(firstline, "#!", 2)) {
			...
			if(interparg)
				tracer_copy_out(p->tracer, interparg, ext_interparg, strlen(interparg) + 1);
			...
			/* rebuild the argv copy it out */
			for(i = argc - 1 + shiftargs; i > 0; i--)
				argv[i] = argv[i - shiftargs];
			argc += shiftargs;
	...
	case SYSCALL32_select:
		if(entering) {
			...
			if(p->syscall_result >= 0) {
				divert_to_dummy(p, p->syscall_result);
				if(prset)
					tracer_copy_out(p->tracer, prset, POINTER(args[1]), nbytes);
				if(pwset)
					tracer_copy_out(p->tracer, pwset, POINTER(args[2]), nbytes);
				if(peset)
					tracer_copy_out(p->tracer, peset, POINTER(args[3]), nbytes);
				if(ptv) {
					COPY_TIMEVAL(tv, ktv);
...
{
	if(p->state == PFS_PROCESS_STATE_DONE)
		return;
