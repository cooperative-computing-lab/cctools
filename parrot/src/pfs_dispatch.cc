...
static int fix_execve ( struct pfs_process *p, uintptr_t old_user_argv, const char *physical_exe, const char *logical_exe, const char *replace_arg0, const char *arg1, const char *arg2 )
{
	...
	uint32_t user_ldso = buffer_pos(&B)+scratch;
	if (ldso[0])
	...
	uint32_t user_argv = buffer_pos(&B)+scratch;
	if (replace_arg0) {
	...
	*/
	switch(p->syscall) {
		...
		case SYSCALL32_socketcall: {
			...
			INT64_T subargs64[6];
			int i;
			tracer_copy_in(p->tracer,subargs,POINTER(args[1]),sizeof(subargs),0);
		...
		case SYSCALL32__llseek:
			if (p->table->isnative(args[0])) {
				...
				if (p->syscall == SYSCALL32__llseek) {
					...
					if (p->syscall_result == 0)
						tracer_copy_out(p->tracer,&p->syscall_result,POINTER(args[3]),sizeof(p->syscall_result),0);
				} else {
			...
			if (args[1] == F_DUPFD || args[1] == F_DUP2FD || args[1] == F_DUPFD_CLOEXEC) {
				...
				switch(cmd) {
					...
					case PFS_SETLKW: {
						...
						tracer_copy_in(p->tracer,&kfl,uaddr,sizeof(kfl),0);
						COPY_FLOCK(kfl,fl);
						...
						if(p->syscall_result<0) {
							...
							COPY_FLOCK(fl,kfl);
							tracer_copy_out(p->tracer,&kfl,uaddr,sizeof(kfl),0);
						}
						divert_to_dummy(p,p->syscall_result);
						break;
					}
					...
					case PFS_SETLKW64: {
						...
						tracer_copy_in(p->tracer,&kfl64,uaddr,sizeof(kfl64),0);
						...
						if(p->syscall_result<0) {
							...
							tracer_copy_out(p->tracer,&kfl64,uaddr,sizeof(kfl64),0);
						}
						divert_to_dummy(p,p->syscall_result);
						break;
					}
		...
		case SYSCALL32_utime:
			if(entering) {
				...
				tracer_copy_in_string(p->tracer,path,POINTER(args[0]),sizeof(path),0);
				if(args[1]) {
					tracer_copy_in(p->tracer,&kut,POINTER(args[1]),sizeof(kut),0);
					COPY_UTIMBUF(kut,ut);
				} else {
