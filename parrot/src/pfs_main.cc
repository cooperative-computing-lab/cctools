...
static void show_use(const char *cmd)
{
	...
	if(pfs_service_lookup("http"))
		printf(" http");
	if(pfs_service_lookup("grow"))
		printf(" grow");
	if(pfs_service_lookup("ftp"))
		printf(" ftp");
	if(pfs_service_lookup("anonftp"))
		printf(" anonftp");
	if(pfs_service_lookup("gsiftp"))
		printf(" gsiftp");
	if(pfs_service_lookup("nest"))
		printf(" nest");
	if(pfs_service_lookup("chirp"))
		printf(" chirp");
	if(pfs_service_lookup("gfal"))
		printf(" gfal lfn guid srm");
	if(pfs_service_lookup("rfio"))
		printf(" rfio");
	if(pfs_service_lookup("dcap"))
		printf(" dcap");
	if(pfs_service_lookup("glite"))
		printf(" glite");
	if(pfs_service_lookup("lfc"))
		printf(" lfc");
	if(pfs_service_lookup("irods"))
		printf(" irods");
	if(pfs_service_lookup("hdfs"))
		printf(" hdfs");
	if(pfs_service_lookup("bxgrid"))
		printf(" bxgrid");
	if(pfs_service_lookup("s3"))
		printf(" s3");
	if(pfs_service_lookup("root"))
		printf(" root");
	if(pfs_service_lookup("xrootd"))
		printf(" xrootd");
...
int main(int argc, char *argv[])
{
	...
	if(s)
		pfs_service_set_block_size(string_metric_parse(s));

	s = getenv("PARROT_MOUNT_FILE");
	if(s)
		pfs_resolve_file_config(s);

	s = getenv("PARROT_MOUNT_STRING");
	if(s)
		pfs_resolve_manual_config(s);
	...
	if(s)
		pfs_master_timeout = string_time_parse(s);
	...
	if(!pfs_file_cache)
		fatal("couldn't setup cache in %s: %s\n", pfs_temp_dir, strerror(errno));
	...
	if(!pfs_channel_init(channel_size * 1024 * 1024))
		fatal("couldn't establish I/O channel");

	if(pfs_use_helper)
		pfs_helper_init(argv[0]);
