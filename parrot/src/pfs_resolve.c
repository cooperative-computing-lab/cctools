...
void pfs_resolve_manual_config(const char *str)
{
	...
	e = strchr(str, '=');
	if(!e)
		fatal("badly formed mount string: %s", str);
	*e = 0;
	e++;
...
void pfs_resolve_file_config(const char *filename)
{
	...
	if(!file)
		fatal("couldn't open mountfile %s: %s\n", filename, strerror(errno));
...
static pfs_resolve_t mount_entry_check(const char *logical_name, const char *prefix, const char *redirect, char *physical_name)
{
	...
	if(
		  ...
		  (!strncmp(prefix, logical_name, plen) && (prefix[plen - 1] == '/' || logical_name[plen] == '/' || plen == llen)
		  )
		) {
		if(!strcmp(redirect, "DENY")) {
		...
		} else if(!strncmp(redirect, "lcache:", 7) && (prefix_sep = strchr(redirect, '|'))) {
			...
			if((!strncmp(logical_name, local_prefix, local_prefix_len)) || (!strncmp(logical_name, pfs_temp_dir, strlen(pfs_temp_dir)))) {
				strcpy(physical_name, logical_name);
				result = PFS_RESOLVE_CHANGED;
			} else {
				int retstat;
...
pfs_resolve_t pfs_resolve(const char *logical_name, char *physical_name, time_t stoptime)
{
	...
	if(!resolve_cache)
		resolve_cache = hash_table_create(0, 0);
