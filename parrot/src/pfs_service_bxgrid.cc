...
const char *bxgrid_lookup_replicaid(MYSQL * mysql_cxn, const char *fileid, int nid, int update = 0)
{
	...
	if(bxgrid_cache_ftor_query) {
		...
		if(!replica_list || replica_list->nreplicas == 0 || nid >= replica_list->nreplicas)
			return NULL;
...
class pfs_service_bxgrid:public pfs_service {
	...
	pfs_service_bxgrid() {
		...
		if(s)
			  bxgrid_cache_stat_query = bxgrid_cache_ftor_query = bxgrid_cache_rtol_query = atoi(s);

		  s = getenv("BXGRID_CACHE_STAT_QUERY");
		if(s)
			  bxgrid_cache_stat_query = atoi(s);

		  s = getenv("BXGRID_CACHE_FTOR_QUERY");
		if(s)
			  bxgrid_cache_ftor_query = atoi(s);

		  s = getenv("BXGRID_CACHE_RTOL_QUERY");
		if(s)
			  bxgrid_cache_rtol_query = atoi(s);

		if(bxgrid_cache_stat_query)
			  bxgrid_stat_query_cache = hash_table_create(0, 0);
		if(bxgrid_cache_ftor_query)
			  bxgrid_ftor_query_cache = hash_table_create(0, 0);
		if(bxgrid_cache_rtol_query)
			  bxgrid_rtol_query_cache = hash_table_create(0, 0);

		  s = getenv("BXGRID_TIMEOUT");
		if(s)
			  bxgrid_timeout = atoi(s);
