...
static int check_implicit_acl(const char *path, int checkflags)
{
	...
	if(stat64(path, &info) == 0) {
		if(info.st_mode & S_IWOTH)
			flags |= IBOX_ACL_WRITE | IBOX_ACL_LIST;
		if(info.st_mode & S_IROTH)
			flags |= IBOX_ACL_READ | IBOX_ACL_LIST;
...
static int pfs_acl_check(pfs_name * name, int flags)
{
	if(!pfs_username)
		return 1;
	if(ibox_acl_check(name->rest, pfs_username, flags))
		return 1;
...
static int pfs_acl_check_dir(pfs_name * name, int flags)
{
	if(!pfs_username)
		return 1;
	if(ibox_acl_check_dir(name->rest, pfs_username, flags))
		return 1;
	return check_implicit_acl(name->rest, flags);
}
...
*/
	...
	virtual int fcntl(int cmd, void *arg) {
		...
		if(cmd == F_SETFL)
			arg = (void *) (((PTRINT_T) arg) | O_NONBLOCK);
...
class pfs_service_local:public pfs_service {
	...
	virtual pfs_file * open(pfs_name * name, int flags, mode_t mode) {
		...
		if(!pfs_acl_check(name, ibox_acl_from_open_flags(flags)))
			  return 0;
	...
	virtual int stat(pfs_name * name, struct pfs_stat *buf) {
		int result;
		struct stat64 lbuf;
		if(!pfs_acl_check(name, IBOX_ACL_LIST))
			return -1;
	...
	virtual int access(pfs_name * name, mode_t mode) {
		int result;
		if(!pfs_acl_check(name, ibox_acl_from_access_flags(mode)))
			return -1;
	...
	virtual int chmod(pfs_name * name, mode_t mode) {
		int result;
		if(!pfs_acl_check(name, IBOX_ACL_WRITE))
			return -1;
	...
	virtual int rename(pfs_name * oldname, pfs_name * newname) {
		int result;
		if(!pfs_acl_check(oldname, IBOX_ACL_READ))
			return -1;
		if(!pfs_acl_check(newname, IBOX_ACL_WRITE))
			return -1;
		if(!pfs_acl_check(newname, IBOX_ACL_WRITE))
			return -1;
	...
	virtual ssize_t getxattr(pfs_name * name, const char *attrname, void *data, size_t size) {
		ssize_t result;
		if(!pfs_acl_check(name, IBOX_ACL_READ))
			return -1;
	...
	virtual int link(pfs_name * oldname, pfs_name * newname) {
		int result;
		if(!pfs_acl_check(oldname, IBOX_ACL_WRITE))
			return -1;
	...
	virtual int mkdir(pfs_name * name, mode_t mode) {
		...
		if(result == 0 && pfs_username)
			ibox_acl_init_copy(name->rest);
	END}
	virtual int rmdir(pfs_name * name) {
		int result;
		if(!pfs_acl_check_dir(name, IBOX_ACL_WRITE))
			return -1;
