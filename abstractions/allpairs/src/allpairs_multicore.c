...
int validate_coordinates(const char *, const char *, int *p, int *q, int *r, int *s);
...
int set_compare_function(const char *function_name)
{
	...
	if(!p)
		return 0;	// Cannot find a registered compare function
	else
		return 1;
}
	...
	while(!feof(setA) && setAPos <= x2) {	// Set A - row of matrix
		...
		while(!feof(setB) && setBPos <= y2) {	// Set B - column of matrix
			...
			if(function_flag == USING_OUTER_FUNCTION) {
					...
					for(j = 0; j < setBCount; j++, k++) {
						if(k == numOfCores) {
							for(l = 0; l < k; l++) {
								...
								debug(D_DEBUG, "cell index: [%d, %d] value: %lf\n", x1 + setAPos - setACount + (countInBlock % numOfAvailableStableElements),
								      y1 + setBPos - setBCount + (int) (countInBlock / numOfAvailableStableElements), resbuff[gnum + countInBlock]);
				...
				for(l = 0; l < k; l++) {
					fscanf(tmpresult[l], "%lf", &(resbuff[countInBlock]));
					debug(D_DEBUG, "cell index: [%d, %d] value: %lf\n", x1 + setAPos - setACount + (countInBlock % numOfAvailableStableElements), y1 + setBPos - setBCount + (int) (countInBlock / numOfAvailableStableElements),
					      resbuff[gnum + countInBlock]);
					fast_pclose(tmpresult[l]);
					countInBlock++;
				}
...
int resolve_block_size(const char *file1, const char *file2, int *x, int *y, int *p, int *q, int *r, int *s)
{
	...
	validate_coordinates(file1, file2, p, q, r, s);
