...
def scan_pkgs():
    ...
    for pkg in [a for a in pkgs if a["channel"] != "pypi"]:
        with open(
            os.path.join(sys.prefix, "conda-meta", pkg["dist_name"] + ".json")
        ) as f:
...
def strip_dots(pkg):
    if pkg.startswith("."):
        raise ImportError(
            "On {}, imports from the current module are not supported".format(pkg)
        )
    return pkg.split(".")[0]
...
def get_stmt_imports(stmt):
    ...
    if isinstance(stmt, ast.Import):
        ...
        if stmt.level != 0:
            raise ImportError(
                "On {}, imports from the current module are not supported".format(
                    stmt.module or "."
                )
            )
...
def create_spec(
    filename, out=None, toplevel=False, function=None, pkg_mapping=[], extra_pkg=[]
):
    overrides = {}
