...
def _download_micromamba(env_dir):
    ...
    try:
        subprocess.run(
            f"curl -Ls https://micro.mamba.pm/api/micromamba/{platform}-{arch}/latest | tar xj -C {env_dir}/bin --strip-components=1 bin/micromamba",
            shell=True,
            check=True,
            capture_output=True,
        )
...
def _pack_env_with_conda_dir(spec, output, ignore_editable_packages=False):
    ...
    try:
        ...
        os.rename(
            f"{spec}/env/bin/poncho_package_run", f"{spec}/bin/poncho_package_run"
        )
        ...
        logger.info(
            "to activate environment run poncho_package_run -e {} <command>".format(
                output
            )
        )
        return output
...
def dict_to_env(
    ...
    pack_env_from_dict(
        spec, output, conda_executable, download_micromamba, ignore_editable_packages
    )
    if cache:
...
def pack_env_from_dict(
    ...
    with tempfile.TemporaryDirectory(prefix="poncho_env") as env_dir:
        ...
        global conda_exec
        (conda_exec, needs_confirmation) = _find_conda_executable(
            conda_executable, env_dir, download_micromamba
        )
    ...
    if result.returncode == 0:
        ...
        for env in env_list["envs"]:
            ...
            if name == env_name:
                return path
    raise Exception(
        f"Cannot find the conda environment named {env_name}. Try checking the spelling or if conda is in your path."
    )
...
def _install_local_pip(env_dir, pip_name, pip_path):
    ...
    process = subprocess.Popen(
        [pip_exec, "-V"], stdout=subprocess.PIPE, stderr=subprocess.PIPE
    )
...
def create_conda_spec(poncho_spec, out_dir, local_pip_pkgs):
    ...
    if "conda" in poncho_spec:
        conda_spec["channels"] = poncho_spec["conda"].get(
            ...
            for dep in list(conda_spec["dependencies"]):
                if isinstance(dep, dict) and "pip" in dep:
                    for pip_dep in list(dep["pip"]):
                        only_name = re.sub(
                            "[!~=<>].*$", "", pip_dep
                        )  # remove possible version from spec
    ...
    for pip_name, location in local_pip_pkgs.items():
        if pip_name not in local_reqs:
            logger.warning(
                "pip package {} was found as pip --editable, but it is not part of the spec. Ignoring local installation.".format(
                    pip_name
                )
            )
