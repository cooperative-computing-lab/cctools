...
int vine_cache_add_file( struct vine_cache *c, const char *cachename, const char *transfer_path, vine_cache_level_t level, int mode, uint64_t size, vine_file_type_t original_type, time_t mtime, timestamp_t start_time, timestamp_t transfer_time, struct link *manager );
