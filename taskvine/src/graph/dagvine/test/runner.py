...
import ndcctools.taskvine.dagvine.dagvine as dagvine_mod
...
def _run_dagvine(graph, n, task_group, port, logs, tag, out_dir, ckpt_dir, priority):
    run_info = logs / tag
    if run_info.exists():
    ...
    dagvine_mod.context_loader_func = context_loader
    try:
        cloudpickle.register_pickle_by_value(gt)
    except Exception:
        pass

    m = DAGVine([port, port], run_info_path=str(logs), run_info_template=tag)
    m.update_params(
        {
...
def run_graph(
    ...
    root = work_root or Path(tempfile.mkdtemp(prefix="dagvine-run-"))
    ...
    try:
        ...
        try:
            os.chdir(TEST_DIR)
            return _run_dagvine(graph, n, task_group, port, logs, tag, out_d, ckpt, priority)
        finally:
