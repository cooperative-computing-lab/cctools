...
void vine_perf_log_write_header(struct vine_manager *q)
{
	...
	fprintf(q->perf_logfile,
			...
			// tasks cumulative
			" tasks_submitted tasks_dispatched tasks_done tasks_failed tasks_successful tasks_cancelled tasks_exhausted_attempts tasks_recovery"
...
void vine_perf_log_write_update(struct vine_manager *q, int force)
{
	...
	buffer_printf(&B, " %d", s.tasks_successful);
