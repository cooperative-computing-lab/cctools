...
struct vine_manager *vine_ssl_create(int port, const char *key, const char *cert)
{
	...
	*/
	...
	q->last_failure_retry_interval = VINE_DEFAULT_LAST_FAILURE_RETRY_INTERVAL;
...
int vine_tune(struct vine_manager *q, const char *name, double value)
{
	...
	} else if (!strcmp(name, "last-failure-retry-interval")) {
		q->last_failure_retry_interval = MAX(0, (int)value);
