...
static vine_result_code_t vine_manager_get_any( struct vine_manager *q, struct vine_worker_info *w, struct vine_task *t, const char *dirname, const char *forced_name, int64_t *totalsize )
{
	char line[VINE_LINE_MAX];
	char name_encoded[VINE_LINE_MAX];
	...
	if(sscanf(line,"file %s %" SCNd64 " 0%o",name_encoded,&size,&mode)==3) {
		...
		if(r==VINE_SUCCESS) *totalsize += size;
...
vine_result_code_t vine_manager_get_output_file( struct vine_manager *q, struct vine_worker_info *w, struct vine_task *t, struct vine_file *f )
{
	...
	if(total_bytes>0) {
		...
		vine_txn_log_write_transfer(q, w, t, f, total_bytes, sum_time, 0, 0);
	}
