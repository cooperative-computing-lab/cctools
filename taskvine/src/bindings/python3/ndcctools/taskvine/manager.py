...
class Manager(object):
    ...
    def hash_function_from_contents(self, func):
        import hashlib
        hash_val = ''
        
        # process the function's code object
        func_co = func.__code__
        hash_val += str(func_co.co_name)
        hash_val += str(func_co.co_argcount)
        hash_val += str(func_co.co_posonlyargcount)
        hash_val += str(func_co.co_kwonlyargcount)
        hash_val += str(func_co.co_nlocals)
        hash_val += str(func_co.co_varnames)
        hash_val += str(func_co.co_cellvars)
        hash_val += str(func_co.co_freevars)
        hash_val += str(func_co.co_code)
        hash_val += str(func_co.co_consts)
        hash_val += str(func_co.co_names)
        hash_val += str(func_co.co_stacksize)

        # compute the hash
        msg = hashlib.md5()
        msg.update(hash_val.encode('utf-8'))
        return msg.hexdigest()

    def create_library_from_functions_v2(self, library_name, exec_mode, function_list, library_context=None, library_context_args=[], library_context_kwargs={}, poncho_env=None, create_poncho_env=False, init_command=None, add_env=True):
        import inspect
        import cloudpickle
        import hashlib
        from ndcctools.poncho.library_network_code_v2 import library_network_code
        from ndcctools.poncho.package_analyze import create_spec_from_functions
       
        # compute a unique id for this library
        lib_hash = ''
        if library_context:
            lib_hash += self.hash_function_from_contents(library_context)
        for func in function_list:
            lib_hash += self.hash_function_from_contents(func.func)
            lib_hash += func.name
            if func.func_context:
                lib_hash += self.hash_function_from_contents(func.func_context)
        lib_hash += library_name
        lib_hash += exec_mode
        lib_hash += str(poncho_env)
        lib_hash += str(init_command)
        lib_hash += str(add_env)
        lib_id = hashlib.md5()
        lib_id.update(lib_hash.encode('utf-8'))
        lib_id = lib_id.hexdigest()
        print(f'lib-id is {lib_id}')

        # if no cache, create a library cache
        # cache only includes the vine library code and blob of package dependencies
        lib_cache_dir = f'{self.cache_directory}/{lib_id}/'
        print(f'lib_cache_dir is {lib_cache_dir}')
        if not os.path.isfile(lib_cache_dir+'cache.complete'):
            print(f'cache for this library is not ready. Building..')

            # remove incomplete library cache to create a fresh one
            if os.path.isdir(lib_cache_dir):
                print(f'removing previously incomplete library cache')
                shutil.rmtree(lib_cache_dir)

            print('making this library\'s cache')
            os.mkdir(lib_cache_dir)

            # script for the library to run
            # this code extraction is really ugly but it happens to work
            library_code_path = lib_cache_dir + 'vine_library_code.py'
            with open(library_code_path, 'w') as f:
                code = inspect.getsource(library_network_code)
                code = "\n".join([line[4:] for line in code.split("\n")[1:]])
                print(code, file=f)
            os.chmod(library_code_path, 0o775)
            print('vine-library-code generated')

            # package all software dependencies of a library
            lib_poncho_env = None
            if create_poncho_env:
                print(f'create_poncho_env is set to {create_poncho_env}. Creating a new poncho env if needed')
                if poncho_env:
                    print(f'a poncho_env is detected to be provided')
                    if poncho_env.endswith('tar.gz'):
                        print('the provided poncho_env fits the expected format and is added to the library')
                        lib_poncho_env = poncho_env
                    else:   # auto package a conda environment
                        print('the poncho_env is given the name of a conda environment. Packaging it')
                        subprocess.run(args=['poncho_package_create', poncho_env, lib_cache_dir + poncho_env + '.tar.gz'])
                        lib_poncho_env = lib_cache_dir + poncho_env + '.tar.gz'
                        print(f'conda environment named {poncho_env} packaged and added to library')
                else:   # scan packages from functions
                    print('creating a poncho env from analyzing functions in this library')
                    func_list = []
                    if library_context:
                        func_list.append(library_context)
                    for func in function_list:
                        func_list.append(func.func)
                        if func.func_context:
                            func_list.append(func.func_context)
                    lib_env_spec = create_spec_from_functions(func_list)
                    print('done extracting a poncho spec. creating one..')
                    subprocess.run(args=['poncho_package_create', lib_env_spec, lib_cache_dir + 'lib_env_spec.tar.gz'])
                    lib_poncho_env = lib_cache_dir + 'lib_env_spec.tar.gz'
                    print('poncho env created')
        else:
            lib_poncho_env = None
            if create_poncho_env:
                if poncho_env:
        ...
        # create an input file containing all information to the library
        # this library info is always created as new and no cache between runs
        lib_info = {}
        lib_info['library_name'] = library_name
        lib_info['library_context'] = [library_context, library_context_args, library_context_kwargs]
        lib_info['exec_mode'] = exec_mode
        lib_info['functions'] = []
        for func in function_list:
            lib_info['functions'].append([func.name, func.func, func.func_context, func.func_context_args, func.func_context_kwargs])
        with open(lib_cache_dir + 'lib_info.cpkl', 'wb') as f:
            cloudpickle.dump(lib_info, f)
        print('library info created')

        # create a LibraryTask
        if init_command:
            t = LibraryTask(f'{init_command} python ./vine_library_code.py', library_name)
        else:
            t = LibraryTask(f'python ./vine_library_code.py', library_name)

        # declare and add library context, functions, and functions' contexts as inputs
        vine_code_file = self.declare_file(lib_cache_dir + 'vine_library_code.py', cache=True, peer_transfer=True)
        t.add_input(vine_code_file, 'vine_library_code.py')
        lib_info_file = self.declare_file(lib_cache_dir + 'lib_info.cpkl', cache=True, peer_transfer=True)
        t.add_input(lib_info_file, 'lib_info.cpkl')
        if add_env:
            lib_poncho_env_file = self.declare_poncho(lib_poncho_env, cache=True, peer_transfer=True)
            t.add_environment(lib_poncho_env_file)

        print('added vine_code, lib_info, and package dependencies to library')
        
        # write cache complete file
        with open(lib_cache_dir + 'cache.complete', 'w') as f:
            print('OK', file=f)
            print('cache complete msg written')
        return t
