...
prepare()
{
	rm -f $STATUS_FILE
	rm -f $PORT_FILE
	rm -rf vine-task-cache-test vine-cache.txlog
...
run()
{
	...
	( ${CCTOOLS_PYTHON_TEST_EXEC} vine_python_cache.py $PORT_FILE; echo $? > $STATUS_FILE) &
...
clean()
{
	rm -f $STATUS_FILE
	rm -f $PORT_FILE
	rm -rf vine-task-cache-test vine-cache.txlog vine-run-info
